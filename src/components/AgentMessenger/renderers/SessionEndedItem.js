...
/**
 * The way out of a thread whose session has closed. It's an offer rather than an
 * automatic restart on purpose: the message that hit the dead session is often a
 * follow-up ("compare it to last season"), and a new session has none of the earlier
 * turns to resolve it against - so re-sending it verbatim would produce a confident
 * answer to the wrong question. Handing the user a new thread with their text in the
 * composer lets them fill in what the fresh session can't know before sending.
 */
